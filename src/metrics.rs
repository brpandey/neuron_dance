...
impl Metrics {
    pub fn new(
        ...
        let metrics_map: HashMap<Mett, bool> =
            metrics_list.into_iter().map(|v| (v, true)).collect();
    ...
    pub fn create_tally(&mut self, batch_type: Option<Batch>, epoch: (usize, usize)) -> Tally {
        Tally::new(
            self.metrics_map.clone(),
            self.cost_fp,
            self.l2_rate,
            batch_type,
            epoch,
...
impl Tally {
    ...
    pub fn t_match(&mut self) {
        // tally or track matches
        if self.metrics_map.contains_key(&Mett::Accuracy) {
            self.total_matches += 1;
        }
    }

    pub fn t_cost(&mut self, a: &Array2<f64>, y: usize) {
        // tally or track cost
    ...
    pub fn regularize_cost(&mut self, w: &Array2<f64>, n_total: usize) {
        if n_total == 0 {
            panic!("total size can't be zero")
        };
    ...
    pub fn summarize(&mut self, n_total: usize) {
        ...
        if self.metrics_map.contains_key(&Mett::Accuracy) {
            self.display_list
                .push(AccuracyM::new_display(self.total_matches, total_size));
            self.total_matches = 0; // reset
        }

        if self.metrics_map.contains_key(&Mett::Cost) {
            self.display_list
                .push(LossM::new_display(self.total_cost, total_size));
    ...
    pub fn display(&self, show: bool) -> VecDeque<String> {
        ...
        let b = self
            .batch_type
            .as_ref()
            .map_or(String::from(""), |v| v.text_display());

        // generate initial metrics texts, {custom metric text} {batch text}
        let mut m_txts: VecDeque<String> = self
            .display_list
            .iter()
            .map(|v| format!("{} {}", v, b))
            .collect();
        ...
        if self.batch_type.as_ref().map_or(false, |b| b.is_mini()) {
            ...
            m_txts = m_txts
                .into_iter()
                .enumerate()
                .map(|(i, t)| if i == 0 { t } else { format!("\t{}", t) })
                .collect();
        }
    ...
    pub fn one_hot(&self, index: usize) -> Option<&Array2<f64>> {
...
mod tests {
    ...
    fn metrics_init(mett: Option<Mett>) -> Metrics {
        use crate::cost::{binary_cross_entropy::BinaryCrossEntropy, Objective};
