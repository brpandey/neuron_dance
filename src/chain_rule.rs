...
enum Layer {
    Output(OutputLayerTerms), // structure for nn's last layer chain rule values
    Hidden(HiddenLayerTerms) // structure for intermediate layer chain rule values
}
...
impl Layer {
    fn acc(&mut self) -> Option<Array2<f64>> {
        match self {
            Layer::Output(l1) => return l1.shared.dc_dz.take(),
            Layer::Hidden(l2) => return l2.shared.dc_dz1.take()
        }
    }
}

pub trait ChainRuleLayer {
    fn chain_rule(&mut self) -> (Array2<f64>, Array2<f64>);
}

impl ChainRuleLayer for OutputLayerTerms {
    fn chain_rule(&mut self) -> (Array2<f64>, Array2<f64>) {
        // Output layer terms
        // dc_db => dc_da * da_dz * dz_db   (or) dc_dz * dz_db
        // dc_dw => dc_da * da_dz * dz_dw.t (or) dc_dz * dz_dw.t
...
impl ChainRuleLayer for HiddenLayerTerms {
    fn chain_rule(&mut self) -> (Array2<f64>, Array2<f64>) {
...
impl ChainRuleLayer for Layer {
    fn chain_rule(&mut self) -> (Array2<f64>, Array2<f64>) {
        match self {
            Layer::Output(l1) => l1.chain_rule(),
            Layer::Hidden(l2) => l2.chain_rule(),
        }
    }
}
