...
impl NAdam {
    ...
    fn bias_correct1(value: &Array2<f64>, decay_rate: f64, gradient: &Array2<f64>) -> Array2<f64> {
        (decay_rate * value / (1.0 - decay_rate)) + ((1.0 - decay_rate) * gradient / (1.0 - decay_rate))
    }
...
impl Optimizer for NAdam {
   ...
   fn calculate<'a>(&mut self, key: ParamKey, value: &'a Array2<f64>, _t: usize) -> Cow<'a, Array2<f64>> {
