...
use crate::types::{SimpleError, Eval};
...
impl TrainTestSubsets {
    pub fn new(format: DataSetFormat, data: TrainTestTuple, class_names: Option<Vec<String>>) -> Self {
        let train = Subset {
            x: data.0,
            y: data.1,
            size: data.2,
    ...
    pub fn min_max_scale(&self, min: f64, max: f64) -> Self {
        ...
        let train = Subset {
            x: x_train_scaled, y: self.train.y.clone(), size: self.train.size,
            format: self.train.format, class_names: self.train.class_names.clone()
        };

        let test = Subset {
            x: x_test_scaled, y: self.test.y.clone(), size: self.test.size,
...
impl fmt::Display for TrainTestSubsets {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "x_train shape {:?}, y_train shape  {:?}, x_test shape {:?}, y_test shape {:?}",
               &self.train.x.shape(), &self.train.y.shape(), &self.test.x.shape(), &self.test.y.shape())
    }
}
